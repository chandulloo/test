...
# 데이터베이스 값 설정
app.config['MYSQL_DATABASE_USER'] = 'CCTV'
app.config['MYSQL_DATABASE_PASSWORD'] = '0623'
app.config['MYSQL_DATABASE_DB'] = 'CCTV'
app.config['MYSQL_DATABASE_HOST'] = 'project-db-stu.ddns.net'
...
def file_upload():
    if request.method == 'POST':
        ...
        sql = "INSERT INTO model_video (image_name, image_dir) VALUES ('%s', '%s')" % (secure_filename(f.filename), 'uploads/'+secure_filename(f.filename))
        cursor.execute(sql)
